...
def packing_fraction(eulerian_class: eulerian_average):
    ...
    for timeDataIndex in range(len(data)):
        ...
        delta = eulerian_class.points_coordinates.T[0:2]
        delta = np.square(delta)
        delta = np.sum(delta.T, axis=1)
        delta = np.sqrt(delta)
        delta = np.ones_like(delta)*0.0015 - delta

        measurement_volume = cylinder_intersection(delta, 0.0015, eulerian_class.radius)
...
def cylinder_intersection(d, R, r):
    ...
    R = np.ones(d.shape)*R
    r = np.ones(d.shape)*r
    volume = np.pi*( 4/3*np.cube(r) - 1/3*np.square(r - d)*(2*R + d))
    return (volume * inRange) + (np.not(inRange) * 
